...
// Fetch data
  ...
  Future<Map<String, dynamic>> _fetchDayoffInfo() async {
    ...
    return {
      ...
      'holidayList': dayoffInfoData['holidayList'],
    };
  }
...
// Function to check if the day is a weekend (Saturday or Sunday)
  ...
  void initState() {
    super.initState();
    _selectedDayoffType =
        '정기휴가'; // Set the default value for _selectedDayoffType
    _fetchData();
          ...
          bottom: PreferredSize(
            ...
            child: Container(
              ...
              color: _showAppBarBorder
                  ? Colors.grey
                  : Colors.transparent, // Dynamic border visibility
            ),
          ),
        ),
  ...
  Widget _buildContent(Map<String, dynamic> data) {
    ...
    final List<Map<String, dynamic>> holidayData = data['holidayList'] ?? [];
    // Convert to a Map<DateTime, String> to store the first holiday name
    ...
    for (var holiday in holidayData) {
      ...
      if (!holidays.containsKey(holidayDate)) {
        // Store only the first holiday name for the given date
        holidays[holidayDate] = holiday["holidayName"];
      }
    }
        ...
        child: Column(children: [
                ...
                children: [
                  TableCalendar(
                    ...
                    headerStyle: HeaderStyle(
                      ...
                      leftChevronVisible: DateTime.now().isAfter(DateTime(
                          DateTime.now().year, DateTime.now().month, 1)),
                      leftChevronIcon: Icon(
                        Icons.chevron_left,
                        color: DateTime.now().month == focusedDay.month
                            ? Colors.grey
                            : Colors.black,
                      ),
                      ...
                      titleTextFormatter: (date, locale) =>
                          '${date.year}.${date.month.toString().padLeft(2, '0')}', // Custom format
                      titleTextStyle: const TextStyle(
                          fontSize: 20,
                          fontWeight: FontWeight.bold), // Style the text
                    ),
                    daysOfWeekStyle: DaysOfWeekStyle(
                      weekdayStyle: TextStyle(
                          color: Colors
                              .black), // Default weekday text color (Mon-Fri)
                      weekendStyle: TextStyle(
                          color: Colors.red), // Weekend text color (Sat-Sun)
                    ),
                    calendarStyle: CalendarStyle(
                      ...
                      todayDecoration: BoxDecoration(
                        color: Colors.yellow.withOpacity(
                            0.5), // Highlight today with a yellow background
                        shape: BoxShape.circle,
                      ),
                      weekendTextStyle: TextStyle(
                          color: Colors.red), // Color weekend dates red,

                      //disabledTextStyle: TextStyle(color: Colors.grey),
                    ),
                    calendarBuilders: CalendarBuilders(
                      //builder preventing from disabledTextStyle by enabledDayPredicate overriding weekendTextStyle
                      disabledBuilder: (context, day, focusedDay) {
                        ...
                        bool isHoliday = holidays.containsKey(
                          ...
                          children: [
                            Container(
                              ...
                              child: Text(
                                '${day.day}',
                                style: TextStyle(
                                  color: (_isWeekend(day) || isHoliday)
                            ...
                            // Holiday name(s) - Updated for single holiday name (not a list)
                            Column(
                              children: [
                                // Display the holiday name for the current day if it exists
                                if (holidayName != null &&
                                    holidayName.isNotEmpty)
                                  Text(
                                    holidayName, // Directly use the holiday name (as a string)
                                    style: TextStyle(
                                      fontSize:
                                          10, // Small text for holiday name
                                      color: Colors
                                          .red, // Use the same red color for consistency
                                    ),
                                  ),
                              ],
                            ),
                    ...
                    enabledDayPredicate: (day) => (!_isWeekend(
                            day) && // Disable weekends
                        !holidays.containsKey(DateTime(
                            day.year, day.month, day.day)) && //Disable holidays
                        !day.isBefore(DateTime.now()) && //Disable before dates
                        !(day.year == DateTime.now().year &&
                            day.month == DateTime.now().month &&
                            day.day == DateTime.now().day)), // Disable  today

                    holidayPredicate: (day) {
                      //print("Checking if ${day.toString()} is a holiday...");
                      day = DateTime(day.year, day.month, day.day);
                      return holidays
                          .containsKey(day); // Check if it's a holiday
                    },
                    ...
                    onPageChanged: (focusedDay) {
                      setState(() {
                        this.focusedDay =
                            focusedDay; // Update focusedDay dynamically
                      });
                    },

                    firstDay: DateTime(DateTime.now().year,
                        DateTime.now().month, 1), // First day of this month
                    lastDay: DateTime(
                        DateTime.now().year + 10,
                        DateTime.now().month,
                        DateTime.now().day), // 10 years later
                  ),
                  ...
                  if (_selectedDates.isNotEmpty)
                    Align(
                      alignment:
                          Alignment.bottomRight, // Align to the bottom-right
                      child: Padding(
                        padding: const EdgeInsets.only(
                            top: 8.0), // Add space between sections
                        child: Text(
                  ...
                  Row(
                    children: [
                      const Icon(Icons.person,
                          color: Colors.black), // Left-sided person icon
                  ...
                  Row(
                    children: [
                      const Icon(Icons.edit,
                          color: Colors.black), // Left-sided writing icon
                      Expanded(
                        child: TextField(
          ...
          _selectedDates.isEmpty || _selectedDayoffType == null
              ? Container(
                  ...
                  color:
                      Colors.grey, // Gray background if conditions are not me
                  padding: EdgeInsets.symmetric(
                      vertical:
                          bottomClickableHeight), // Add some vertical space
